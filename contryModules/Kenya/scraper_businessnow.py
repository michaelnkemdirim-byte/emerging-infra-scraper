#!/usr/bin/env python3
"""
Scraper for Business Now Kenya
Website: https://businessnow.co.ke
Type: WordPress news site
Focus: Business, economy, finance, energy, technology
"""
...
# Configuration
BASE_URL = 'https://businessnow.co.ke'
...
def determine_category(title, summary):
    ...
    if any(kw in text for kw in ['airport', 'terminal', 'runway', 'aviation']):
        ...
        return 'rail'
    elif any(kw in text for kw in ['port', 'harbor', 'maritime', 'shipping']):
        return 'port'
        ...
        return 'SEZ'
    elif any(kw in text for kw in ['power', 'electricity', 'energy', 'solar', 'renewable', 'thermal power', 'nuclear', 'wind power', 'hydroelectric', 'geothermal']):
        return 'energy'
...
def scrape_businessnow():
    """Main scraping function for Business Now"""
    print("="*70)
    print("Scraping Business Now Kenya (businessnow.co.ke)")
    print("="*70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Scrape Business Now for infrastructure news')
    parser.add_argument('--output', default='businessnow_data.csv', help='Output CSV file')
    args = parser.parse_args()

    data = scrape_businessnow()
