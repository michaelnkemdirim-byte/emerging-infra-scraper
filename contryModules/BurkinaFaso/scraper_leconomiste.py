#!/usr/bin/env python3
"""
Scraper for L\'Economiste du Faso (Burkina Faso)
Website: https://leconomistedufaso.com
...
def search_leconomiste(keyword: str) -> list:
    """Search L\'Economiste du Faso WordPress API for a keyword"""
    try:
        params = {
            'search': keyword,
            'after': DATE_AFTER,
...
def scrape_article(article_id: int, url: str, link_title: str) -> dict:
    """Scrape individual article from L\'Economiste du Faso"""
    try:
        # Fetch full article details from API using article ID
        article_url = f"https://leconomistedufaso.com/wp-json/wp/v2/posts/{article_id}?_fields=id,title,date,link,excerpt,content"
...
def scrape_leconomiste():
    """Main scraping function for L\'Economiste du Faso"""
    print("="*70)
    print("Scraping L\'Economiste du Faso (Burkina Faso)")
    print("="*70)
    ...
    for keyword, category in KEYWORD_CATEGORY_MAP.items():
        ...
        articles = search_leconomiste(keyword)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Scrape L\'Economiste du Faso for infrastructure news')
    parser.add_argument('--output', default='leconomiste_data.csv', help='Output CSV file')
    parser.add_argument('--workers', type=int, default=15, help='Parallel workers (default: 15)')
    args = parser.parse_args()

    MAX_WORKERS = args.workers
    data = scrape_leconomiste()
