#!/usr/bin/env python3
"""
Scraper for KT Press Rwanda
Website: https://www.ktpress.rw
Type: WordPress news site
Focus: Business, economy, infrastructure, technology, energy
"""
...
def determine_category(title, summary):
    ...
    if any(kw in text for kw in ['airport', 'terminal', 'runway', 'aviation', 'bugesera']):
        return 'airport'
    elif any(kw in text for kw in ['railway', 'rail', 'train', 'metro']):
        return 'rail'
    elif any(kw in text for kw in ['port', 'harbor', 'harbour', 'maritime', 'shipping', 'lake kivu']):
        return 'port'
        ...
        return 'SEZ'
    elif any(kw in text for kw in ['power', 'electricity', 'energy', 'solar', 'renewable', 'thermal power', 'nuclear', 'wind power', 'hydroelectric', 'cooking gas', 'methane']):
        return 'energy'
...
def scrape_ktpress():
    """Main scraping function for KT Press"""
    print("="*70)
    print("Scraping KT Press Rwanda (ktpress.rw)")
    print("="*70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Scrape KT Press for infrastructure news')
    parser.add_argument('--output', default='ktpress_data.csv', help='Output CSV file')
