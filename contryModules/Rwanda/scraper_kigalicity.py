#!/usr/bin/env python3
"""
City of Kigali - News Scraper
Scrapes news articles from kigalicity.gov.rw (current year only)
"""
...
# Configuration
BASE_URL = "https://www.kigalicity.gov.rw"
NEWS_LIST_URL = f"{BASE_URL}/news"
COUNTRY = "Rwanda"
SOURCE_NAME = "City of Kigali"
...
def fetch_news_list_page(page_num: int = 1) -> tuple:
    ...
    try:
        ...
        # Find all article links with news-detail pattern
        for link in soup.find_all('a', href=True):
            ...
            if '/news-detail/' in href:
                ...
                if date_year and date_year != CURRENT_YEAR:
                    continue
        ...
        return unique_articles, len(unique_articles) > 0
...
def scrape_all_news():
    """Scrape all news articles from kigalicity.gov.rw (current year only)"""
    ...
    while page_num <= max_pages:
        ...
        articles, has_content = fetch_news_list_page(page_num)

        if not has_content or not articles:
            print(f"\n  No more {CURRENT_YEAR} articles found at page {page_num}")
            break

        all_articles.extend(articles)
        print(f"  Page {page_num}: Found {len(articles)} articles from {CURRENT_YEAR} (total: {len(all_articles)})", end='\r')

        page_num += 1
        time.sleep(0.5)  # Rate limiting

    print(f"\n  Total {CURRENT_YEAR} article URLs collected: {len(all_articles)}")

    if not all_articles:
        print(f"  ERROR: No articles from {CURRENT_YEAR} found!")
        return []
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Scrape City of Kigali news (current year)')
    parser.add_argument('--output', '-o', default='kigalicity_data.csv', help='Output CSV file')
    ...
    if data:
        ...
        print(f"No articles from {CURRENT_YEAR} collected!")
