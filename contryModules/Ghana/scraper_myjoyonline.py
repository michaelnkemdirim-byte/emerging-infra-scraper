#!/usr/bin/env python3
"""
MyJoyOnline - Economic & Business News Scraper
...
# Category filtering - Only fetch from relevant categories
# Business (2), Economy (4), Banking & Finance (41), Energy (5)
RELEVANT_CATEGORIES = [2, 4, 41, 5]
...
def fetch_posts_from_api(page: int = 1, per_page: int = 100, categories: list = None) -> tuple:
...
def scrape_all_posts():
    """Scrape all posts from MyJoyOnline via WordPress API"""
    print(f"Starting scraper for {SOURCE_NAME}")
    print("="*60)

    # Step 1: Get first page to determine total pages (with category filter)
    print("\nStep 1: Fetching posts from WordPress API (Business, Economy, Finance, Energy categories)...")
    posts_data, total_pages, total_posts = fetch_posts_from_api(page=1, per_page=100, categories=RELEVANT_CATEGORIES)
    ...
    if max_pages > 1:
        ...
        for page in range(2, max_pages + 1):
            ...
            posts_data, _, _ = fetch_posts_from_api(page=page, per_page=100, categories=RELEVANT_CATEGORIES)
            if posts_data:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Scrape MyJoyOnline')
    parser.add_argument('--output', '-o', default='myjoyonline_data.csv', help='Output CSV file')
